...
async function getBrowserInstance() {
  // if browser is not connected, clear it
...
export async function getServerSideProps(context) {
  ...
  const isBot =
    userAgent.match(
      "/bot|facebook|embed|got|firefox\/92|firefox\/38|curl|wget|go-http|yahoo|generator|whatsapp|preview|link|proxy|vkshare|images|analyzer|index|crawl|spider|python|cfnetwork|node/gi|Discordbot",
    ) !== null;
  ...
  if (
    !fs.existsSync(volumePATH + "logs/" + id + ".json") ||
    !fs.existsSync(volumePATH + "img/" + id + ".png")
  ) {
    await writeLogData(id);
  }

  var log_data = JSON.parse(
    fs.readFileSync(volumePATH + "logs/" + id + ".json"),
  );
